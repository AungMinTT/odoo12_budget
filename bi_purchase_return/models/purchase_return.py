...
class BiPurchaseReturn(models.Model):
    _name = 'bi.purchase.return'
                ...
                if location_id:
                    for location in location_id:
                        ...
                        type_obj = self.env['stock.picking.type'].search([
                            ('default_location_src_id', '=', order.location_id.id),
                            ('code', '=', 'outgoing')
                        ])
                        if type_obj:
    ...
    return_line = fields.One2many('bi.purchase.return.line', 'return_id', string='Order Lines', copy=True)
    ...
    default_location_dest_id_usage = fields.Selection(related='picking_type_id.default_location_src_id.usage', string='Destination Location Type', readonly=True)
    ...
    def create(self, vals):
        if vals.get('name', _('New')) == _('New'):
            if 'company_id' in vals:
                vals['name'] = self.env['ir.sequence'].with_context(force_company=vals['company_id']).next_by_code('bi.purchase.return') or _('New')
            else:
                vals['name'] = self.env['ir.sequence'].next_by_code('bi.purchase.return') or _('New')
        return super(BiPurchaseReturn, self).create(vals)

    @api.multi
    def unlink(self):
        for po_return in self:
            if po_return.state == 'return' and po_return.is_delivered and po_return.invoice_id:
                raise UserError(_('You cannot delete this record since delivery order and invoice have been already created.'))
            po_return.return_line.unlink()
        return super(BiPurchaseReturn, self).unlink()
        ...
        if not self.picking_id:
            ...
            for line in self.return_line:
                move_values = {
                    ...
                    'name': line.name,
                    'company_id': self.picking_type_id.default_location_src_id.company_id.id,
    ...
    def create_refund(self):
        ...
        if not self.invoice_id:
            ...
            for each_line in self.return_line:
                if each_line.product_id.property_account_expense_id:
                    line_account_id = each_line.product_id.property_account_expense_id.id
                else:
                    line_account_id = each_line.product_id.categ_id.property_account_expense_categ_id.id
                line_values = {
                    ...
                    'invoice_line_tax_ids': [(6, 0, each_line.product_id.supplier_taxes_id.ids)],
                }
...
class BiPurchaseReturnLine(models.Model):
    _name = 'bi.purchase.return.line'
    ...
    return_id = fields.Many2one('bi.purchase.return', string='Order Reference', index=True, required=True, ondelete='cascade')
    ...
    def onchange_product_id(self):
        ...
        if self.product_id:
            if self.product_id.supplier_taxes_id:
                self.tax_ids = [(6, 0, self.product_id.supplier_taxes_id.ids)]
